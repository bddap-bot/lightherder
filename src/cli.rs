...
pub struct Cli {
    ...
    /// `None` is whatever the rig is wired to.
    pub seed: Option<Input>,
...
fn seed(value: &str) -> Result<Input, String> {
    value.parse().map_err(|why| format!("--seed {why}"))
}
...
mod tests {
    ...
    fn a_seed_that_names_no_source_says_so() {
        for bad in ["/dev/video0", "v4l2:", ":/dev/video0", ":", ""] {
            let why = parse_argv(&[&format!("--seed={bad}")]).unwrap_err();
            assert!(
                why.starts_with("--seed ") && why.contains("FORMAT:NAME"),
                "{why}"
            );
        }
